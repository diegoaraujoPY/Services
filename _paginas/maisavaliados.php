...
<header>
	<nav class="navegacao">
		<?php
			...
			if(!isset($_SESSION['id_usuario'])){
				...
				echo		"<li class='nav-item'>";
				echo			"<a class='nav-link' href='tipo.html'><img class='navbar-brand' src='../_icones/entre.png' alt='' width='22' height='32'>Entre</a>";
				echo		"</li>";
				echo		"<li class='nav-item'>";
				echo			"<a class='nav-link' href='tipo.html'><img class='navbar-brand' src='../_icones/notificacoes.png' alt='' width='22' height='32'>Notificações</a>";
				echo		"</li>";
				echo	"</ul>";
				echo "</div>";
			}
		...
		<div class="row">
			...
			<div class="container">
				<form class="d-flex justify-content-center pesquisa" method="get" action='busca.php'>
					...
					<input class='form-control me-6' type='search' name='q' placeholder='Buscar profissionais' aria-label='Search'>";
	...
	<div class="container">
        <div role="main" class="col-md-10">
			...
			$sql_code = "SELECT * FROM profissional p JOIN servico s ON p.id_profissional = s.id_profissional ORDER BY nota DESC LIMIT 10";
			$sql_query = $mysqli->query($sql_code) or die("Falha na execução do código SQL" . $msqli->error);
			
			echo "<h4 class='fontserv'> <b>OS 10 PROFISSIONAIS MAIS AVALIADOS DA PLATAFORMA!</b> </h4>";
				...
				echo "<span style='padding: 30px; color: red; font-size: 20px; font-family: Verdana; text-align: left;'>NENHUM SERVIÇO ESTÁ CADASTRADO AINDA!</span>";
					...
					while ($servico = mysqli_fetch_array($sql_query)){
						...
						$id_categoria_servico = $servico['id_categoria'];
							...
							echo 	"<div class='container justify-content-end avaliados'>";
							echo 		"<div class='row'>";
							...
							echo "</div>";
							echo "</div>";
							echo 	"<div class='col' style='margin: 10px; text-align: right;'>";
