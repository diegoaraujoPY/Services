	...
	<div class="wrapper fadeInDown">
		<div id="formContent">
			...
			<form method="POST" action="">
				<input type="text" id="login" class="fadeIn second" name="id_usuario" placeholder="Usuário" required>
				<input type="password" id="password" class="fadeIn third" name="senha" placeholder="Senha" required>
				<input type="submit" class="fadeIn fourth" value="Entrar">
			</form>
			<div id="formFooter">
				<a class="underlineHover" href="registrar_usuario.php">Registrar-se</a>
			</div>
		</div>
	</div>
...
<?php
	...
	if(isset($_POST['id_usuario']) || isset($_POST['senha'])){
		...
		if(strlen($_POST['id_usuario'])== 0){
			...
			$sql_code = "SELECT * FROM usuario WHERE id_usuario = '$id_usuario' AND senha = '$senha'";
			...
			if($quantidade == 1){
				...
				header("Location: ../_dashboard/main_usuario.php");
				
			} else {
				$sql_code_adm = "SELECT * FROM adm WHERE usuario = '$id_usuario' LIMIT 1";
