...
<html lang="pt-br">
	<head>
		<title><?php $nome = $_SESSION['nome_adm']; echo "$nome - Avaliações Cadastradas";?></title></title>
		<meta charset="UTF-8"/>
		...
		<div class="adm-painel">
			<div class="row">
				<?php
					...
					echo "<span style='padding: 30px; color: #16578b; font-size: 30px; font-family: Verdana; font-weight: bold; text-align: center;'>AVALIAÇÕES</span>";
				
					echo "<div class='container'>";
				
					include('../_conexao/conexao_profissional.php');
					$sql_avaliacao = 'SELECT * FROM avaliacao AS a JOIN servico AS s ON a.id_servico = s.id_servico JOIN usuario AS u ON a.id_usuario = u.id_usuario ORDER BY s.especialidade';
					...
					echo "<th scope='col'>Comentário</th>";
					...
					while($avaliacao = mysqli_fetch_array($sql_query_avaliacao)){
						...
						$id_categoria = $avaliacao['id_categoria'];
						...
						echo 	"<tr>";
						echo 		"<td class='col-2' style='font-size: 10pt; padding: 10px; text-align: left;'>$nome_usuario $sobrenome_usuario</td>";
						echo 		"<td class='col-1' style='font-size: 10pt; padding: 10px; text-align: left;'>$especialidade</td>";
						echo 		"<td class='col-4' style='font-size: 10pt; padding: 10px; text-align: left;'>$comentario</td>";
						echo 		"<td class='col-2' style='font-size: 10pt; padding: 10px; text-align: left;'>";
						echo 		"<div class='estrelas'>";
									
						include_once('../_funcoes/avaliacao_estrelas.php');
								
						echo exibirAvaliacao($qualidade);
						
						$resultado_format = number_format($qualidade, 1, '.', '');
						
						echo 			"<span style='font-family: Verdana; font-size: 16px;'>$resultado_format</span><br>";
						echo 		"</div>";
						echo 		"</td>";
						echo 		"<td class='col-1' style='font-size: 10pt; padding: 10px; text-align: left;'><a class='abutao' style='margin: 10px; background: red;' href='adm009-excluir.php?id_av=$id_avaliacao'>Excluir</a></td>";
						echo 	"</tr>";
					}
					echo 	"<tbody>";
					echo  "</table>";
