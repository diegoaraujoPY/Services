...
<html lang="pt-br">
	<head>
		<title><?php $nome = $_SESSION['nome_adm']; echo "$nome - Serviços Cadastrados";?></title></title>
		<meta charset="UTF-8"/>
		...
		<div class="adm-painel">
			<div class="row">
				<?php
					...
					echo "<span style='padding: 30px; color: #16578b; font-size: 30px; font-family: Verdana; font-weight: bold; text-align: center;'>SERVIÇOS CADASTRADOS</span>";
				
					echo "<div class='container'>";
				
					include('../_conexao/conexao_profissional.php');
					$sql_usuario = 'SELECT * FROM servico AS s JOIN profissional AS p ON s.id_profissional = p.id_profissional ORDER BY s.especialidade';
					...
					echo "<th scope='col'>Profissional</th>";
					...
					while($usuarios_cadastrados = mysqli_fetch_array($sql_query_usuarios)){
						...
						$especialidade = $usuarios_cadastrados['especialidade'];
						$id_servico = $usuarios_cadastrados['id_servico'];
						$sobrenome_usuario = $usuarios_cadastrados['sobrenome'];
						$telefone = $usuarios_cadastrados['telefone'];
						$id_categoria = $usuarios_cadastrados['id_categoria'];
						
						$sql_cat = "SELECT * FROM categoria WHERE id_categoria = '$id_categoria'";
						$sql_query_cat = $mysqli->query($sql_cat) or die ("Falha na execução do código SQL " . $mysqli->error);
						
						echo 	"<tr>";
						echo 		"<td class='col-1' style='font-size: 10pt; padding: 10px; text-align: left;'>$id_servico</td>";
						echo 		"<td class='col-2' style='font-size: 10pt; padding: 10px; text-align: left;'>$especialidade</td>";
						echo 		"<td class='col-3' style='font-size: 10pt; padding: 10px; text-align: left;'>$nome_usuario $sobrenome_usuario</td>";
						while($categoria = mysqli_fetch_array($sql_query_cat)){
							$categoria_nome = $categoria['nome'];
							echo 		"<td class='col-2' style='font-size: 10pt; padding: 10px; text-align: left;'>$categoria_nome</td>";
						}
						echo 		"<td class='col-2' style='font-size: 10pt; padding: 10px; text-align: left;'><a class='abutao' style='margin: 10px; background: blue;' href='adm007-editar_servico.php?id_serv=$id_servico'>Editar</a>";
						echo 		"<a class='abutao' style='margin: 10px; background: red;' href='adm007-excluir.php?id_serv=$id_servico'>Excluir</a></td>";
						echo 	"</tr>";
					}
					echo 	"<tbody>";
					echo  "</table>";
