...
<html lang="pt-br">
	<head>
		<title><?php $nome = $_SESSION['nome_adm']; echo "$nome - Contratos Andamento";?></title></title>
		<meta charset="UTF-8"/>
		...
		<div class="adm-painel">
			<div class="row">
				<?php
					...
					echo "<span style='padding: 30px; color: #16578b; font-size: 30px; font-family: Verdana; font-weight: bold; text-align: center;'>CONTRATOS EM ANDAMENTO</span>";
				
					echo "<div class='container'>";
				
					include('../_conexao/conexao_profissional.php');
					$sql_contrato = 'SELECT * FROM contrato AS c JOIN servico AS s ON c.id_servico = s.id_servico JOIN usuario AS u ON c.id_usuario = u.id_usuario ORDER BY s.especialidade';
					...
					echo "<th scope='col'>Serviço Prestado</th>";
					echo "<th scope='col'>Cliente</th>";
					echo "<th scope='col'>Data Início</th>";
					echo "<th scope='col'>Opções</th>";
					echo "</tr>";
					echo "</thead>";
					
					while($contrato = mysqli_fetch_array($sql_query_contrato)){
						$nome_usuario = $contrato['nome'];
						$id_contrato = $contrato['id_contrato'];
						$especialidade = $contrato['especialidade'];
						$id_servico = $contrato['id_servico'];
						$sobrenome_usuario = $contrato['sobrenome'];
						...
						echo 		"<td class='col-3' style='font-size: 10pt; padding: 10px; text-align: left;'>$data</td>";
						echo 		"<td class='col-1' style='font-size: 10pt; padding: 10px; text-align: left;'><a class='abutao' style='margin: 10px; background: red;' href='adm008-excluir.php?id_cont=$id_contrato'>Excluir</a></td>";
						echo 	"</tr>";
					}
					echo 	"<tbody>";
					echo  "</table>";
