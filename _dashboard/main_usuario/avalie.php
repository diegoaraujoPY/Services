	...
	<div class="content painel">
		<div class="main-painel">
			<div class="row">
				<?php
					$id_usuario = $_SESSION['id_usuario'];
					echo "<span style='font-size: 35px; font-family: Verdana; color: #16578b; font-weight: bold; text-align: center;'>AVALIAÇÃO DE SERVIÇOS!</span>";
							
					include("../../_conexao/conexao_profissional.php");
					
					$sql_contrato = "SELECT * FROM contrato AS c JOIN servico AS s ON c.id_servico = s.id_servico JOIN profissional AS p ON s.id_profissional = p.id_profissional WHERE id_usuario = $id_usuario ORDER BY c.data_final DESC";
					$sql_query_contrato = $mysqli->query($sql_contrato)or die("Falha na execução do código SQL" . $msqli->error);
					
					$quantidade = $sql_query_contrato->num_rows;
					
					if($quantidade == 0){
				
						echo "<span style='padding: 30px; color: red; font-size: 20px; font-family: Verdana; text-align: left;'>NENHUM SERVIÇO NO MOMENTO!</span>";
					...
					echo "<form action='../main_profissional.php' style='padding: 10px;'>";
							...
							while ($servico = mysqli_fetch_array($sql_query_contrato)){
								...
								echo 				"<span style='font-family: Verdana; font-size: 18px; text-align: left;'><b>Profissional:</b> $nome_profissional $sobrenome_profissional</span><br>";
								
									if($data_servico==0){
										echo "<span style='font-family: Verdana; font-size: 18px; text-align: left; color: red;'><b>VOCÊ CANCELOU ESSE SERVIÇO!</b></span><br>";
										echo "</div>";
									} else {
										echo "<span style='font-family: Verdana; font-size: 18px; text-align: left;'><b>Iniciado em:</b> $data_servico</span><br>";
								
									if($data_fim=='0'){
										echo 		"<span style='font-family: Verdana; font-size: 18px; text-align: left;'><b>SERVIÇO EM ANDAMENTO</b></span><br>";
										echo 	"</div>";
										
										echo "<script language=javascript>";
										echo "function confirmar_cancelar(){";
										echo 	"if (confirm('Cancelar serviço?')){";
										echo 		"alert('Serviço cancelado!!');";
										echo 		"window.location.replace('../manipulacao/cancelar_servico.php?c=$id_servico');";
										echo 	"} else {";
										echo 		"window.location.replace('avalie.php');";
										echo 	"}";
										echo "}";
										echo "</script>";
										
										echo 		"<div style='text-align: right; padding: 10px;'>";
										echo 			"<a class='abutao' style='margin: 10px; background: rgba(173,14,14,1);' onclick='confirmar_cancelar()' href=''>Cancelar</a>";
										echo 		"</div>";
										...
										$sql_avaliacao = "SELECT * FROM avaliacao WHERE id_usuario = $id_usuario AND id_servico = $id_servico";
										$sql_query_avaliacao = $mysqli->query($sql_avaliacao)or die("Falha na execução do código SQL" . $msqli->error);
					
										$quant_avaliacao = $sql_query_avaliacao->num_rows;
										
										if($quant_avaliacao==0){
											echo 		"<div style='text-align: right; padding: 10px;'>";
											echo 			"<a class='abutao' style='margin: 10px' href='../avaliacao/avaliando_servico.php?cont=$id_servico'>AVALIAR SERVIÇO</a>";
											echo 		"</div>";
										} else {
											
											while($avaliacao = mysqli_fetch_array($sql_query_avaliacao)){
												$id_avaliacao = $avaliacao['id_av'];
												
												echo 		"<div style='text-align: right; padding: 10px;'>";
												echo 			"<a class='abutao' style='margin: 10px; background: rgba(173,14,14,1);' href='../avaliacao/manter_avaliacao.php?c=$id_servico'>Manter</a>";
												echo 			"<a class='abutao' style='margin: 10px;' href='../avaliacao/editar_avaliação.php?a=$id_avaliacao&s=$id_servico'>Editar Avaliação</a>";
												echo 		"</div>";
											}
										}
