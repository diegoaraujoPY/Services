		...
		<div class="main-painel">
			<div class="row">
				<?php
					...
					$sql_code = "SELECT * FROM servico AS s JOIN categoria AS c ON s.id_categoria = c.id_categoria WHERE id_servico = '$cod_servico'";
					...
					$_SESSION['id_servico'] = $cod_servico;
					
					$sql_categoria = "SELECT * FROM categoria";
					$sql_query_categoria = $mysqli->query($sql_categoria) or die("Falha na execução do código SQL" . $msqli->error);
					...
					echo "<form method='POST' action='editaservico.php'>";
					echo 	"<div class='mb-1' style='text-align: left;'>";
					echo 		"<label for='especialidade' class='form-label'>ESPECIALIDADE</label><br>";
					echo 		"<input type='text' name='especialidade_servico'  class='form-control' placeholder='Pedreiro, Eletricista, Encanador...' value='$especialidade' id='especialidade' required>";
					echo 	"</div>";
					echo 	"<div class='mb-1' style='text-align: left;'>";
					...
					echo 		"</select>";
					echo 	"</div>";
					echo 	"<br><button type='submit' style='width: 75px; height: 43px; margin: 10px;' class='btn btn-success'>Salvar</button>";
					echo 	"<a class='abutaoeditar' style='margin: 10px;' href='editarregioesdeatendimento.php'>Regiões</a>";
					echo 	"<a class='abutaoexcluir' style='margin: 10px;' href='myservicos.php'>Cancelar</a>";
					echo 	"</form>";
