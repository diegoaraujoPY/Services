		...
		<div class="main-painel">
			<div class="row">
					<?php
						...
						$sql_disponivel = "SELECT * FROM contrato AS c JOIN servico AS s ON c.id_servico = s.id_servico WHERE s.id_profissional = '$id_profissional' AND c.data_final = '0'";
						$sql_query = $mysqli->query($sql_disponivel) or die("Falha na execução do código SQL" . $msqli->error);
						...
						if($quant==0){
							...
							echo "<span style='font-size: 20px; font-family: Verdana; color: #16578b; font-weight: bold;'>STATUS DE PROFISSIONAL</span>";
							if($status_profissional=='DISPONÍVEL'){
								echo "<div class='col'>";
								echo 	"<form method='POST' action='alterardisponibilidade.php'>";
								echo 		"<input type='radio' id='disponivel' name='statusn' value='DISPONÍVEL' checked='checked'/>";
								echo 		"<label style='padding: 10px' for='disponivel'>Disponível</label>";
								echo 		"<input type='radio' id='indisponivel' name='statusn' value='INDISPONÍVEL'/>";
								echo 		"<label style='padding: 10px' for='indisponivel'>Indisponível</label><br>";
								echo 		"<br><button type='submit' class='btn btn-success'>SALVAR</button>";
								echo 	"</form>";
								echo "</div>";
							} else {
								echo "<div class='col'>";
								echo 	"<form method='POST' action='alterardisponibilidade.php'>";
								echo 		"<input type='radio' id='disponivel' name='statusn' value='DISPONÍVEL'/>";
								echo 		"<label style='padding: 10px' for='disponivel'>Disponível</label>";
								echo 		"<input type='radio' id='indisponivel' name='statusn' value='INDISPONÍVEL' checked='checked'/>";
								echo 		"<label style='padding: 10px' for='indisponivel'>Indisponível</label><br>";
								echo 		"<br><button type='submit' class='btn btn-success'>SALVAR</button>";
								echo 	"</form>";
								echo "</div>";
							...
							echo "<span style='font-size: 20px; font-family: Verdana; color: #16578b; font-weight: bold;'>STATUS DE PROFISSIONAL</span>";
							echo "<div class='col'>";
							echo 	"<form method='POST' action='../main_profissional.php'>";
							echo 		"<input disabled type='radio' id='disponivel' name='statusn' value='DISPONÍVEL'/>";
							echo 		"<label style='padding: 10px' for='disponivel'>Disponível</label>";
							echo 		"<input disabled type='radio' id='indisponivel' name='statusn' value='INDISPONÍVEL' checked='checked'/>";
							echo 		"<label style='padding: 10px' for='indisponivel'>Indisponível</label><br>";
							echo 		"<br><button type='submit' class='btn btn-success'>SALVAR</button>";
							echo 	"</form>";
							echo "</div>";
