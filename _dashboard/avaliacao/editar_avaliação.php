					...
					echo 				"<div id='notificationsBody' class='notifications'>";
												...
												while($servico_avaliar = mysqli_fetch_array($sql_query_avalie)){								
													...
													if($quant_avaliacao==0){
														...
														echo 			"<a class='abutaoexcluir' style='margin: 10px;' href='manter_avaliacao.php?c=$id_servico'>Manter</a>";
														echo 			"<a class='abutao' style='margin: 10px;' href='editar_avaliação.php?a=$id_avaliacao&s=$id_servico'>Editar Avaliação</a>";
														echo 		"</div>";
														echo 	"</div>";
														echo 	"</div>";
														}
		...
		<div class="main-painel">
			<div class="row">
				<?php
					...
					$id_avaliacao = $_GET['a'];
					$id_servico = $_GET['s'];
					$_SESSION['id_avalie'] = $id_avaliacao;
					$_SESSION['id_servico'] = $id_servico;
					
					include("../../_conexao/conexao_profissional.php"); 
					
					$sql_code_avaliacao = "SELECT * FROM avaliacao AS a JOIN usuario AS u ON a.id_usuario = u.id_usuario WHERE id_av = $id_avaliacao";
					$sql_query_avaliacao = $mysqli->query($sql_code_avaliacao) or die("Falha na execução do código SQL" . $msqli->error);
					
					while($avaliacao_do_servico = mysqli_fetch_array($sql_query_avaliacao)){
						
						$comentario = $avaliacao_do_servico['comentario'];
						$qualidade = $avaliacao_do_servico['qualidade'];
						$recomendo = $avaliacao_do_servico['recomendacao'];
					
						echo "<span style='font-size: 35px; font-family: Verdana; color: #16578b; font-weight: bold; text-align: center;'>AVALIAÇÃO</span>";
						echo "<form method='POST' action='editandoavaliacao.php'>";
						...
						echo editarAvaliacao($qualidade);
						...
						if($recomendo=='SIM'){
							echo 	"<div class='mb-1' style='text-align: left;'>";
							echo 		"<input class='form-check-input' type='radio' name='recomendo' value='SIM' id='sim' checked required>";
							...
							echo 	"</div>";
							echo 	"<div class='mb-1' style='text-align: left;'>";
							echo 		"<input class='form-check-input' type='radio' name='recomendo' value='NÃO' checked id='nao'>";
							echo 		"<label class='form-check-label' for='nao'>Não</label>";
							echo 	"</div>";
						}
